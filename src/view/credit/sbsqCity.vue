<template>
  <div class="container bgF">
    ...
    <div class="selectCity">
      <dl v-for='(data,index) of datas' id="data.letter" :class='{ floatLetter : classPower === index}' :key=index>
        <dt><a :name="data.letter" :ref='data.letter'>{{data.letter}}</a></dt>
        <dd v-for='(city,index) in data.citys' :key=index>{{city}}</dd>
      </dl>
      <p ref="floatLeft">
        <a v-for='(data,index) of datas' @click='goPosition(index)' :key=index>{{data.letter}}</a>
      </p>
    </div>
  </div>
</template>
