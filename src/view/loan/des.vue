<template>
  <div class="container">
    ...
    <template v-for='(data,index) of datas'>
      <p class="listTop" :key=index><span class="col3">{{data.tit}}</span>
        <template v-if='data.num !== 100'>(还可输入{{data.num}}字)</template>
      </p>
      <div class="feedBack" :key=index>
        <div class="feed-text feed-mini" :class='{ col5 : data.def!==data.txt }'>
          <textarea maxlength="100" class="limitFeed" @focus='textFocus(index)' @blur='textBlur(index)' v-model.trim='data.txt'></textarea>
        </div>
      </div>
    </template>
  </div>
